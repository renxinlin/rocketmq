...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            ...
            case CREATE_JUST:
                ...
                // 核心 获取mq客户端管理器[缓存表有] 封装netty
                this.mQClientFactory = MQClientManager.getInstance().getOrCreateMQClientInstance(this.defaultMQProducer, rpcHook);
                // 注册当前生产者到客户端实例中 producerTable《group producer》
                ...
                if (startFactory) {
                    // 启动客户端通信实例 启动netty 定时任务  拉取消息服务和负载均衡
    ...
    private SendResult sendDefaultImpl(
        ...
        //step 2  获取发送消息的相关主题路由信息  （以及从name server 获取主题信息）
        ...
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            ...
            for (; times < timesTotal; times++) {
                ...
                // step 3 轮询 选择broker 选择message queue   多主多从其实是分片主从
                ...
                if (mqSelected != null) {
                    ...
                    try {
                        ...
                        // step 5 处理 sendLatencyFaultEnable对应的功能 发送失败 isolation为true代表被隔离  false 延迟【endTimestamp - beginTimestampPrev】
    ...
    /**
     * 代码@1：从本地缓存(ConcurrentMap< String , TopicPublishInfo>)中尝试获取，第一次肯定为空，走代码@2的流程。
     * 代码@2：尝试从 NameServer 获取配置信息并更新本地缓存配置。
     * 代码@3：如果找到可用的路由信息并返回。
     * 代码@4：如果未找到路由信息，则再次尝试使用默认的 topic去找路由配置信息
     *
     * @param topic
     * @return
     */
    ...
    private SendResult sendKernelImpl(final Message msg,
        ...
        if (brokerAddr != null) {
            ...
            try {
                ...
                //  禁止钩子执行 没有默认实现 我个人认为作者的意思是说 你自定义一个实现类
                // 根据这个CheckForbiddenContext 上下文 或者 你自己的逻辑判断要不要停止发送
                //  如果停止发送 则抛出异常
